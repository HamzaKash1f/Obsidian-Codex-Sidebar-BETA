...
export default class MyPlugin extends Plugin {
	...
	async onload() {
		...
		this.addCommand({
			...
			callback: async () => {
				...
				if (first) {
					// If it exists, close/detach it (toggle off)
					first.detach();
					return;
				}

				// Otherwise open in right sidebar (toggle on)
		...
		// Keep sample commands if you want (optional)
		this.addCommand({
			id: "replace-selected",
	...
	onunload() {
		this.app.workspace.getLeavesOfType(CODEX_VIEW_TYPE).forEach((leaf) => leaf.detach());
	}
